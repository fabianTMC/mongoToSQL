module.exports = {
    ...
    UNSUPPORTED_SEARCH_VALUE: function(value) {
        return "`" + JSON.stringify(value) + "` is not a supported $search type"
    },
    ...
    NOT_A_VALID_DATE_MATH_FORMAT: "Given comparison value is not in the correct date comparison format",
    INVALID_DATE_OR_TIME: "Given date/time value is not valid",
    INVALID_COMPARISON_TYPE: function(type) {
        return "`" + JSON.stringify(type) + "` is not a valid comparison operator"
    },
    NOT_AN_ARRAY: (type) => `An array was not passed to the ${type.toLowerCase()} operator`,
