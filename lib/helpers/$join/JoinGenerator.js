...
module.exports = function joinGenerator($lookup, tableName, tables, joinCondition, unified = false) {
    ...
    if(unified) {
        tables += ` ${join} ` + mysql.escapeId($lookup.from) + " ON " + mysql.escapeId($lookup.from) + "." + mysql.escapeId($lookup.foreignField) + " = " + mysql.escapeId($lookup.on || tableName) + "." + mysql.escapeId($lookup.localField);
    } else {
